...
// default constructor
se3 u1;
Eigen::Matrix<double,6,1> Identity = Eigen::Matrix<double,6,1>::Zero();
...
Eigen::Matrix<double,4,4> V1;
V1.setRandom();
V1 = (V1+Eigen::Matrix<double,4,4>(V1.transpose())); // Symmetric matrix
...
se3 u2(Eigen::Matrix<double, 6,1>::Random());
se3 u3(Eigen::Matrix<double, 6,1>::Random());
se3 u4(Eigen::Matrix<double, 6,1>::Random());
...
ASSERT_EQ(m2,se3::SSM(x));


// Adjoint test

Eigen::Matrix<double,6,6> m3 = Eigen::Matrix<double,6,6>::Zero();
m3.block(0,0,3,3) = se3::SSM(u2.th_);
m3.block(3,3,3,3) = se3::SSM(u2.th_);
m3.block(0,3,3,3) = se3::SSM(u2.p_);
...
// Log test
ASSERT_LT((se3::Log(u4.Exp())-u4.data_).norm(),kse3_threshold_);
...
Eigen::Matrix<double,6,1> v1 = Eigen::Matrix<double,6,1>::Random();
Eigen::Matrix<double,6,1> v2 = Eigen::Matrix<double,6,1>::Random();
Eigen::Matrix<double,6,1> v3 = Eigen::Matrix<double,6,1>::Random();
Eigen::Matrix<double,6,1> v4 = Eigen::Matrix<double,6,1>::Random();
...
Eigen::Matrix<double,6,1> e3;
Eigen::Matrix<double,6,1> e4;
Eigen::Matrix<double,6,1> e5;
Eigen::Matrix<double,6,1> e6;
Eigen::Matrix<double,6,6> estimated_jr_inv;
e1 << 1,0,0,0,0,0;
e2 << 0,1,0,0,0,0;
e3 << 0,0,1,0,0,0;
e4 << 0,0,0,1,0,0;
...
estimated_jr_inv.block(0,0,6,1) = (se3(se3::Log(u1.Exp()*se3(e1*dt).Exp())).Vee() - v1)/dt;
estimated_jr_inv.block(0,1,6,1) = (se3(se3::Log(u1.Exp()*se3(e2*dt).Exp())).Vee() - v1)/dt;
estimated_jr_inv.block(0,2,6,1) = (se3(se3::Log(u1.Exp()*se3(e3*dt).Exp())).Vee() - v1)/dt;
estimated_jr_inv.block(0,3,6,1) = (se3(se3::Log(u1.Exp()*se3(e4*dt).Exp())).Vee() - v1)/dt;
estimated_jr_inv.block(0,4,6,1) = (se3(se3::Log(u1.Exp()*se3(e5*dt).Exp())).Vee() - v1)/dt;
estimated_jr_inv.block(0,5,6,1) = (se3(se3::Log(u1.Exp()*se3(e6*dt).Exp())).Vee() - v1)/dt;
...
// Test Jr
ASSERT_LE( (u1.Jr()*u1.JrInv()-Eigen::Matrix<double,6,6>::Identity()).norm(),kse3_threshold_) << "Error with the right Jacobian";

// Test Jl
ASSERT_LE( (u1.Jl()*u1.JlInv()-Eigen::Matrix<double,6,6>::Identity()).norm(),kse3_threshold_) << "Error with the right Jacobian";
